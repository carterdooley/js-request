...
//REMEMBER TO START SERVER IN CONSOLE. CD TO DIRECTORY THE RUN 'NODE SERVER/INDEX.JS'
...
// sayHelloButton.addEventListener("mouseover", changeBtnClr)
...
const repeatMyParam = () => {
    ...
    .then(response => {
        ...
        repeatText.style.backgroundColor = 'purple';
...
const queryFunct = () => {
    axios.get('http://localhost:3000/query-test/?age=10')
    .then(res => {
...
document.getElementById('query-button').addEventListener('click', queryFunct)
...
const createFood = (event) => {
    ...
    .then(res => {
        ...
        // let p = document.createElement('p')
        //  p.textContent = res.data.textContent
        //     document.querySelector('body').append(p)
    })
}
